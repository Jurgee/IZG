...
void drawLine(int x1, int y1, int x2, int y2, S_RGBA color)
{
    ...
    auto k{ (dy << FRAC_BITS) / dx };
        ...
        // #2 : Doplnte kod pro upravu koordinatu pro ruzne kvadranty, pripadne upravte i setPixel(...).
    for (int x = x1; x <= x2; x++)
    {
        if (sflag)
            setPixel(y >> FRAC_BITS, x, COLOR_GREEN);
        else
            setPixel(x, y >> FRAC_BITS, COLOR_GREEN);
        y += k;
    }
...
void put8PixelsOfCircle(int x, int y, int s1, int s2, S_RGBA color)
{
     ...
     * ╰────┻────╯
     */
    setPixel(s1 + x, s2 + y, COLOR_GREEN);
    setPixel(s1 - x, s2 - y, COLOR_GREEN);
    setPixel(s1 + x, s2 - y, COLOR_GREEN);
    setPixel(s1 - x, s2 + y, COLOR_GREEN);
    setPixel(s1 + y, s2 + x, COLOR_GREEN);
    setPixel(s1 - y, s2 - x, COLOR_GREEN);
    setPixel(s1 + y, s2 - x, COLOR_GREEN);
    setPixel(s1 - y, s2 + x, COLOR_GREEN);
}
